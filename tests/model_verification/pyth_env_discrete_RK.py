...
class VehicleDynamicsData:
    def __init__(self):
        self.vehicle_params = dict(
            ...
            b=3.9 - 1.49634995,  # Distance between the CG of the tractor and its rear axle [m]
            lhtt=3 - 1.49634995,  # Distance between the hitch point and the center of gravity (CG) of the tractor [m]
            lhtl=7.03106945 - 3,  # Distance between the hitch point and the CG of the semitrailer [m]
            d=9.3 - 7.03106945,  # Distance between the rear axle and the CG of the semitrailer [m]
            hs_tt=1.12317534 - 0.2,  # Height of the CG of the sprung mass to the roll center for the tractor [m]
            hs_tl=0.933400843 - 0.3,  # Height of the CG of the sprung mass to the roll center for the semitrailer [m]
            hh_tt=1.07 - 0.2,  # Height of hitch point to the roll center  for the tractor [m]
            hh_tl=1.07 - 0.3,  # Height of hitch point to the roll center for the semitrailer [m]
            ...
            Izz_tl=179992,  # Yaw moment of inertia of the whole mass of the semitrailer [kg m^2]
            Ixx_tl=9959.7,  # Roll moment of inertia of the sprung mass of the semitrailer [kg m^2]
            ...
            kvarphi_tt=1500 / 3.14 * 180 * 6,  # # roll stiffness of tire (front)[N/m]
            kvarphi_tl=3000 / 3.14 * 180 * 4,  # roll stiffness of tire (rear)[N/m]
        ...
        self.Att43 = -self.kvarphi_tt + self.ms_tt * self.gravity * self.hs_tt
        self.Att44 = -self.cvarphi_tt
        ...
        self.Atl43 = -self.kvarphi_tl + self.ms_tl * self.gravity * self.hs_tl
        self.Atl44 = -self.cvarphi_tl
    ...
    def dynamic_func(self, states, actions):
        ...
        A_matrix = block_diag(np.matmul(np.linalg.inv(Mtt_matrix), Att_matrix),
                              np.matmul(np.linalg.inv(Mtl_matrix), Atl_matrix))

        B_matrix = block_diag(np.matmul(np.linalg.inv(Mtt_matrix), Btt_matrix),
                              np.matmul(np.linalg.inv(Mtl_matrix), Btl_matrix))

        C_matrix = block_diag(np.matmul(np.linalg.inv(Mtt_matrix), Ctt_matrix),
                              np.matmul(np.linalg.inv(Mtl_matrix), Ctl_matrix))
...
class Semitruckpu6dof(PythBaseEnv):
        ...
        **kwargs,
        ...
        if work_space is None:
            ...
            init_high = np.array([2, 2, 0.1, 2, 2, 2, 0.1, 2,
                                  0.1, 0.1, 0.01, 0.1, 0.1, 0.1, 0.01, 0.1], dtype=np.float32)
        ...
        self.action_space = gym.spaces.Box(
            low=np.array([0, 0, -max_steer]),
            high=np.array([5000, 5000, max_steer]),
            dtype=np.float32,
        )
    ...
    def reset(
        ...
        **kwargs,
        ...
        self.state = np.concatenate(
            (self.ref_points[0] + delta_state[:8], delta_state[8:]))#np.array([0, 0,0, 20, 0, 0, 0, 20])
        obs = self.get_obs()
    ...
    def compute_reward(self, action: np.ndarray) -> float:
        ...
        v2, phi2_dot, varphi2, varphi2_dot = self.state
        ...
        return -(
            ...
            + 0.5 * varphi1_dot ** 2
            + 0.4 * steer ** 2
            + 0.1 * (steer - self.action_last[2]) ** 2
            + 0.4 * ((action[0]/400) ** 2+(action[1]/400) ** 2)
            + 0.1 * ((action[0]-self.action_last[0])/400 ** 2+
                           (action[1]-self.action_last[1])/400 ** 2)
        )
