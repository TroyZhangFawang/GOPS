...
def read_csv(root_path):
    ...
    delta = np.array(data_result.iloc[start_index:end_index:interval, 35], dtype='float32')/180*math.pi/40 # rad
    pos_x = np.array(data_result.iloc[start_index:end_index:interval, 20], dtype='float32')
    pos_y = np.array(data_result.iloc[start_index:end_index:interval, 21], dtype='float32')
    phi = np.array(data_result.iloc[start_index:end_index:interval, 9], dtype='float32')
    vx = np.array(data_result.iloc[start_index:end_index:interval, 10], dtype='float32')
    vy = np.array(data_result.iloc[start_index:end_index:interval, 41], dtype='float32')
    phidot = np.array(data_result.iloc[start_index:end_index:interval, 13], dtype='float32')/180*math.pi
    varphhi = np.array(data_result.iloc[start_index:end_index:interval, 34], dtype='float32')
    varphhi_dot = np.array(data_result.iloc[start_index:end_index:interval, 12], dtype='float32')/180*math.pi

    torque_cmd = np.array(data_result.iloc[start_index:end_index:interval, 39], dtype='float32')/4
    deltaf_cmd = np.array(data_result.iloc[start_index:end_index:interval, 38], dtype='float32')/180*math.pi/40 # rad
    data_pool = np.zeros((len(step_list), 17))
...
def model_verification_4wisd(real_data,env_id):
    ...
    for i in range(1, run_step):
        ...
        steering_angle_rad = real_data[i, 16]
        control = np.array([real_data[i, 15], real_data[i, 15],
                            real_data[i, 15], real_data[i, 15],
                            steering_angle_rad])
        print("road_info", road_info)

        state_python = model_self.vehicle_dynamics.f_xu(state_python, control, delta_t, road_info)
        # state_python[3] = real_data[i, 4]
    ...
    picture_dir = "plot_4wisd_kinematic/"
    ...
    l3, = plt.plot(np.arange(1, run_step, 1) * delta_t, Qw1_cmd, lw=2, color="green")
    plt.legend(handles=[l1, l2, l3], labels=['4wisd', 'car', 'cmd'], prop={'size': 10}, loc=2,
               ncol=2)
    ...
    l3, = plt.plot(np.arange(1, run_step, 1) * delta_t, delta_w1_cmd, lw=2, color="g")
    plt.legend(handles=[l1, l2, l3], labels=['4wisd', 'car', 'cmd'], prop={'size': 10}, loc=2,
               ncol=2)
    ...
    f8 = plt.figure("-ax", figsize=(8, 5))
    ax = f8.add_axes([0.1, 0.11, 0.87, 0.86])  # [left, bottom, width, height]
    l1, = plt.plot(np.arange(1, run_step, 1)*delta_t, ax_self, lw=2, color="darkviolet")
    l2, = plt.plot(np.arange(1, run_step, 1)*delta_t, ax_car, lw=2, linestyle='--', color="deepskyblue")
    plt.legend(handles=[l1, l2], labels=['4wisd', 'car'], prop={'size': 10}, loc=2,
               ncol=2)
    plt.ylabel(r"$a_x$ [m/s^2]", fontsize=14)
    plt.xlabel("Times [s]", fontsize=14)
    ...
    l3, = plt.plot(np.arange(1, run_step, 1) * delta_t, Qw3_cmd, lw=2, color="g")
    plt.legend(handles=[l1, l2, l3], labels=['4wisd', 'car', 'cmd'], prop={'size': 10}, loc=2,
               ncol=2)
    ...
    l3, = plt.plot(np.arange(1, run_step, 1) * delta_t, Qw4_cmd, lw=2, color="g")
    plt.legend(handles=[l1, l2, l3], labels=['4wisd', 'car', 'cmd'], prop={'size': 10}, loc=2, ncol=2)
