...
class PythEnergybimodalplanning2a(PythBaseEnv):
    ...
    def __init__(
            ...
            lateral_sample: float = 4.5,  # 横向采样距离
    ...
    def reset(
        ...
        # add static obstacle
        self.static_obss = []
        static_time = [3, 4.5, 6, 7.5, 9, 10.5, 12, 13.5, 15, 16.5]
        for i_static in range(self.static_obstacle_num):
            ...
            self.static_length = np.array(
                [0.5, 3, 5, 0.5, 3, 5, 0.5, 3, 5, 7])  # np.random.uniform(0, 2, self.static_obstacle_num)
            self.static_width = np.array([5.0, 4, 3.0, 5.6, 4.7, 3.9, 1.7, 1.4, 3.2,
                                          6.7])  # np.random.uniform(0, 2, self.static_obstacle_num)#, 0.5
            self.static_height = np.array([0.2, 0.5, 0.5, 0.2, 0.5, 0.5, 0.2, 0.1, 0.5,
                                           1.0])  # np.random.uniform(0, 1, self.static_obstacle_num)#, 0.23
    ...
    def get_obs(self) -> np.ndarray:
        ...
        # ]
        ref_obs = np.stack((ref_x_tf, ref_y_tf, ref_phi_tf, ref_u_tf, self.ref_points[:, 4], self.ref_points[:, 5]), 1)[
                  1:].flatten()
    ...
    def update_static_state(self):
        for i, static_obs in enumerate(self.static_obss):
            self.static_state[i] = np.array(
                [static_obs.obs_id, static_obs.x, static_obs.y, static_obs.phi, static_obs.length, static_obs.width,
                 static_obs.height, static_obs.material],
                dtype=np.float32,
            )
    ...
    def generate_bezier_curves(self, obstacle: StaticObstacle) -> List[BezierCurve]:
        ...
        current_pos = self.state[:2]
        # 横向采样点
        lateral_offsets = [-self.lateral_sample - obstacle.width / 2 - self.veh_width / 2, 0,
                           self.lateral_sample + obstacle.width / 2 + self.veh_width / 2]
        mid_points = [
        ...
        # 终点位置（统一使用前方采样点）
        end_point = [
            np.array([obstacle.x + self.forward_sample + obstacle.length / 2, obstacle.y + offset]) for offset in
            lateral_offsets
        ]
