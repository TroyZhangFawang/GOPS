...
class PythEnergybimodalplanning2aModel(PythBaseModel):
    ...
    def _generate_new_ref_point(self, batch_size, state, path_num, u_num, slope_num, t, t_start, t_end, generate_guide,
                                next_ref_points, best_curve_isnone):
    ...
    def get_bezier_guide_point(self, curve, t_interpolate, state, t_start: torch.Tensor, t_end: torch.Tensor, path_num,
                               u_num):
        # 确保时间参数在有效范围内
        if t_end <= t_start:
    ...
    def generate_bezier_curves(self, state: torch.Tensor, obstacle: StaticObstacle, batch_idx: int) -> list:
        ...
        mid_points = torch.stack([
            torch.tensor([obstacle.x[batch_idx,], obstacle.y[batch_idx,] - lateral_sample - obstacle.width[
                batch_idx,] / 2 - self.veh_width / 2], dtype=torch.float32),  # 左侧
            torch.tensor([obstacle.x[batch_idx,], obstacle.y[batch_idx,]], dtype=torch.float32),  # 中心
            torch.tensor([obstacle.x[batch_idx,], obstacle.y[batch_idx,] + lateral_sample + obstacle.width[
                batch_idx,] / 2 + self.veh_width / 2], dtype=torch.float32)  # 右侧
        ])

        # 纵向采样点 - 使用tensor
        end_points = torch.stack([
            torch.tensor([obstacle.x[batch_idx,] + forward_sample + obstacle.length[batch_idx,] / 2,
                          obstacle.y[batch_idx,] - lateral_sample - obstacle.width[
                              batch_idx,] / 2 - self.veh_width / 2],
                         dtype=torch.float32),
            torch.tensor(
                [obstacle.x[batch_idx,] + forward_sample + obstacle.length[batch_idx,] / 2, obstacle.y[batch_idx,]],
                dtype=torch.float32),
            torch.tensor([obstacle.x[batch_idx,] + forward_sample + obstacle.length[batch_idx,] / 2,
                          obstacle.y[batch_idx,] + lateral_sample + obstacle.width[
                              batch_idx,] / 2 + self.veh_width / 2],
