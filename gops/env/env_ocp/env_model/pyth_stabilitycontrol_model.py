...
#
#  Description: 4WS 4WD vehicle stability control model environment
...
class VehicleDynamicsModel(VehicleDynamicsData):
    def __init__(self):
        ...
        self.rho = 1.206  #air mass density
        self.Cd = 0.3  # coefficient of air force
    ...
    def f_xu(self, state, action, delta_t, road_info):
        ...
        R = torch.tensor([theta_road, varphi_road]).reshape(2, 1)
        ...
        for batch in range(self.batch_size):
            ...
            B_matrix, Lc_matrix), Ec_matrix), Mw_matrix), temp)++torch.matmul(R_matrix, R)).squeeze()
            X_dot[0] = X_dot[0]-1/(2*self.m)*self.Cd*self.rho*self.A*v_x**2-self.g*np.sin(theta_road)
...
class FourwsdvehiclestabilitycontrolModel(PythBaseModel):
    def __init__(
        self,
        ref_vx: float = 10,
        pre_horizon: int = 30,
    ...
    def forward(
        ...
        reward = self.compute_reward(obs, action, ref_points)

        next_state = self.vehicle_dynamics.f_xu(state, action, self.dt, slope_points)
...
def env_model_creator(**kwargs):
    ...
    """
    return FourwsdvehiclestabilitycontrolModel(**kwargs)
