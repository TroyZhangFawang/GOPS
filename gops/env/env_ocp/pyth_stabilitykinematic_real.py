...
class VehicleDynamicsData:
    def __init__(self):
        self.vehicle_params = dict(
            ...
            A=3.3,  # Front area
            rho=1.2258, #air mass density
            Cd=0.2,  # coefficient of air force
            g=9.81,
            Rw=0.426,
            Iw=3.1,  # wheel spin inertia [kg m2]
            f_rolling=0.018,  # rolling resistence coefficient
            mu_r=0.8,  # Road adhesion coefficient
            lw=1.8,
        ...
        self.f_rolling = self.vehicle_params["f_rolling"]
        ...
        self.tau_Q = self.vehicle_params["tau_Q"]
        self.tau_deltaf = self.vehicle_params["tau_deltaf"]
...
class Fourwdstabilitycontrol(PythBaseEnv):
    ...
    def step(self, action: np.ndarray) -> Tuple[np.ndarray, float, bool, dict]:
        ...
        action_psc = self.state[8:] + action
