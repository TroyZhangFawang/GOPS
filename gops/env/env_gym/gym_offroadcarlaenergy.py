...
try:
    ...
    # carla_path = "./recources/dist/carla-0.9.6-py3.6-linux-x86_64.egg"
...
class CarlaOffroadEnv:
    ...
    def update_epinfos(self, measurement):
        ...
        roll_rate = measurement['angular_velocity'].x/180*3.14
        print("roll_rate[rad/s]",roll_rate)
        speed = math.sqrt(v.x ** 2 + v.y ** 2 + v.z ** 2)
        location = np.array([t.location.x, t.location.y]) #, t.location.z
        car_heading = t.rotation.yaw/180*3.14
        car_roll = t.rotation.roll / 180 * 3.14
        print("car_roll[rad]",car_roll)
        ...
        longitudinal_slope = self.calculate_longitudinal_slope(current_wp_index, self.waypoints_manager)
        bank_angle = self.calculate_lateral_slope(current_wp_index, self.waypoints_manager)
        print("road_slope[rad]",longitudinal_slope, "bank_angle[rad]",bank_angle)
        # mileage
    ...
    def calculate_longitudinal_slope(self, current_wp_index, waypoints_manager, look_ahead=3):
        """
        计算沿道路方向的纵向坡度
        :param current_wp_index: 当前waypoint索引
        :param waypoints_manager: waypoints管理对象
        :param look_ahead: 向前看的waypoint数量
        :return: 纵向坡度(弧度)
        """
        current_wp = waypoints_manager.get_wp(current_wp_index, key='middle', d=3)
        next_wp_index = min(current_wp_index + look_ahead, waypoints_manager.points_num - 1)
        next_wp = waypoints_manager.get_wp(next_wp_index, key='middle', d=3)

        # 计算高度差和水平距离
        delta_z = next_wp[2] - current_wp[2]
        delta_xy = ((current_wp[0]-next_wp[0])**2+(current_wp[1]-next_wp[1])**2)**0.5

        # 避免除以零
        if delta_xy < 0.001:
            return 0.0

        longitudinal_slope = math.atan2(delta_z, delta_xy)
        return longitudinal_slope

    def calculate_lateral_slope(self, current_wp_index, waypoints_manager, lateral_distance=1.0):
        """
        基于左右边界点计算横向坡度
        :param waypoints: (N,11)的waypoints数组
        :param current_idx: 当前索引
        :return: 横向坡度(弧度)
        """
        # 获取当前点的左右边界高度
        current_wp_side1 = waypoints_manager.get_wp(current_wp_index, key='side1', d=3)
        current_wp_side2 = waypoints_manager.get_wp(current_wp_index, key='side2', d=3)
        current_wp_middle = waypoints_manager.get_wp(current_wp_index, key='middle', d=3)
        left_z = current_wp_side1[2]  # 左边界z坐标
        right_z = current_wp_side2[2]  # 右边界z坐标
        center_z = current_wp_middle[2]  # 中心线z坐标

        # 计算左右边界到中心线的距离
        left_x, left_y = current_wp_side1[0], current_wp_side1[1]
        right_x, right_y = current_wp_side2[0], current_wp_side2[1]
        center_x, center_y = current_wp_middle[0], current_wp_middle[1]

        left_dist = math.sqrt((left_x - center_x) ** 2 + (left_y - center_y) ** 2)
        right_dist = math.sqrt((right_x - center_x) ** 2 + (right_y - center_y) ** 2)

        # 计算横向坡度 (考虑两侧不对称情况)
        if left_dist + right_dist < 0.001:
            return 0.0

        # 加权平均坡度
        left_slope = math.atan2(center_z - left_z, left_dist) if left_dist > 0 else 0
        right_slope = math.atan2(right_z - center_z, right_dist) if right_dist > 0 else 0
        return (left_slope * left_dist + right_slope * right_dist) / (left_dist + right_dist)
class Env(CarlaOffroadEnv):
    ...
    def __init__(self, **kwargs):
        ...
        port=kwargs['port'],
        server_size=(4, 3),
        city_name='Offroad_8', #kwargs['city_name']
