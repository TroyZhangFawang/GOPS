...
class Waypoints:
    ...
    def __init__(self, city_name):
        try:
            self.raw_waypoints = pd.read_csv("/home/bit/Troy/1_code/gops-for-offroad/gops/env/env_gym/recources/waypoints/" + self.file_mapping[city_name.lower()])
        except:
    ...
    def get_wp(self, idx, key='middle', d=2):
        if type(idx) == list or type(idx) == tuple:
            ...
            if idx >= self.points_num: idx=self.points_num-1
            ...
            data = data[:d]
            return data
    def get_global_wp(self, idx, length):
        idx_list = [idx +i for i in range(length)]
        wp_list = self.get_wp(idx_list, key='middle', d=2)
        return np.array(wp_list).reshape((-1, 2))
