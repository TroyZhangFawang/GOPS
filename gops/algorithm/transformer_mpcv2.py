...
class TRANSFORMERMpcv2(AlgorithmBase):
    ...
    def _compute_loss_policy(self, data):
        ...
        info_init = data
        o_ori = o
        a_ori = self.networks.policy.get_all_action(o_ori)
        d_ori = d
        v_pi = 0
        total_loss = []
        for step in range(self.forward_step):
            ...
            tt = 0
            if step == 0:
                o, r, d, info = self.envmodel.forward(o, a[:, step, :], d, info_init)
                v_pi = r
                loss = v_pi.mean()
                total_loss.append(loss)
                v_pi = 0
            else:
                for virtual_t in range(step, -1, -1):
                    a = a[:, virtual_t, :]
                    o, r, d, info = self.envmodel.forward(o, a, d, info)
                    v_pi += r * (self.gamma**tt)
                    tt += 1
                    o_clip = o[:, :self.state_dim+virtual_t*self.ref_obs_dim]
                    a = self.networks.policy.get_all_action(o_clip)
                    # print("step:", step)
                    # print("Check a: ", torch.isnan(a).any())
                    # print("Check o: ", torch.isnan(o).any())
