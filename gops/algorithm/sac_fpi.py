...
__all__ = ["ApproxContainer", "SACFpi"]
...
class SACFpi(AlgorithmBase):
    ...
    def __compute_gradient(self, data: DataDict, iteration: int):
        ...
        loss_policy, entropy, policy_loss1, policy_loss2, log_barrier, feasible = self.__compute_loss_policy(data)
        ...
        self.step += 1
        if self.step % self.t_update_delay == 0:
            self.t = min(self.max_t, self.t * self.t_increase_factor)
        else:
            self.t = self.t
    ...
    def __compute_loss_policy(self, data: DataDict):
        ...
        return loss_policy, entropy, policy_loss1, policy_loss2, log_barrier, feasible
