...
class RMPC2(AlgorithmBase):
    ...
    def _compute_loss_policy(self, data):
        ...
        # random_len = torch.randint(1, self.forward_step + 1, (o.shape[0],))
        ...
        random_len = torch.multinomial(weights, num_samples=o.shape[0], replacement=True) + 1
        steps = torch.arange(self.forward_step).expand(batch_size, self.forward_step)
        gamma_powers = self.gamma ** steps
        mask = steps < (random_len).unsqueeze(1)
        info = data
        v_pi = torch.zeros((batch_size, self.forward_step))
        a = self.networks.policy.forward_all_policy(o, lengths=random_len)
