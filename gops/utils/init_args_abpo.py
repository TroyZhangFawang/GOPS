...
def init_args(env, **args):
    ...
    if args["save_folder_upper"] is None:
        dir_path = os.path.dirname(__file__)
        dir_path = os.path.dirname(dir_path)
        dir_path = os.path.dirname(dir_path)
        args["save_folder_upper"] = os.path.join(
            dir_path + "/results/",args["env_id"],
            args["algorithm"] +'_'+
            datetime.datetime.now().strftime("%y%m%d-%H%M%S")+ "-upper_" + str(args["max_iteration_upper"]) + "-inner_" + str(
                args["max_iteration"]),
        )
    os.makedirs(args["save_folder_upper"], exist_ok=True)
    os.makedirs(args["save_folder_upper"] + "/apprfunc", exist_ok=True)
    os.makedirs(args["save_folder_upper"] + "/evaluator", exist_ok=True)

    # set random seed
    seed = args.get("seed", None)
    args["seed"] = seed_everything(seed)
    print("Set global seed to {}".format(args["seed"]))
    with open(args["save_folder_upper"] + "/config.json", "w", encoding="utf-8") as f:
