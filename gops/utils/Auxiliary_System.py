...
class OCSys:
    ...
    def setDyn(self, ode):
        if not hasattr(self, 'auxvar'): #如果没有self.auxvar则执行self.setAuxvarVariable()
    ...
    def ocSolver(self, ini_state, horizon, auxvar_value=1, print_level=0, costate_option=1):
        ...
        if costate_option == 0:
            ...
            dfx_fun = casadi.Function('dfx', [self.state, self.control, self.auxvar], [jacobian(self.dyn, self.state)]) #dfx
            dhx_fun = casadi.Function('dhx', [self.state, self.auxvar], [jacobian(self.final_cost, self.state)]) #dh(final cost)x
            dcx_fun = casadi.Function('dcx', [self.state, self.control, self.auxvar], #dcx
    ...
    def ocSolver_ref(self, Path_Cost_Update, ini_state, horizon, ref_traj, auxvar_value=1, print_level=0, costate_option=1):
        assert hasattr(self, 'state'), "Define the state variable first!"
        assert hasattr(self, 'control'), "Define the control variable first!"
        assert hasattr(self, 'dyn'), "Define the system dynamics first!"
        assert hasattr(self, 'path_cost'), "Define the running cost function first!"
        ...
        for k in range(horizon):
            ...
            # Integrate till the end of the interval
            Xnext = self.dyn_fn(Xk, Uk, auxvar_value)
            path_cost, _ = Path_Cost_Update(ref_traj[k])
            self.setPathCost(path_cost)
            Ck = self.path_cost_fn(Xk, Uk, auxvar_value)
            J = J + Ck
        ...
        # Adding the final cost
        _, final_cost = Path_Cost_Update(ref_traj[horizon])
        self.setFinalCost(final_cost)
        J = J + self.final_cost_fn(Xk, auxvar_value)
...
class LQR:
    ...
    def lqrSolver(self, ini_state, horizon):
        ...
        for t in range(self.horizon - 1, 0, -1):
            P_next = PP[t]
            W_next = WW[t]
            invHuu = numpy.linalg.inv(Huu[t]) # 矩阵求逆
